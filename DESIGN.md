...
| **Principal binding (ENS_ENSIP26)** | Resolve the ENS name from the agent's principal binding, verify that name's `agent-endpoint[ans]` text record names one of the agent's ANSNames | Record matches the ANSName of an ACTIVE version of the agent | Only when `principalBinding.type` is `ENS_ENSIP26` |
