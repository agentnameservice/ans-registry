...
| **Principal binding (ENS_ENSIP26)** | Resolve the ENS name from the agent's principal binding, verify that name's `agent-endpoint[ans]` text record equals the agent's ANSName | Record matches the agent's current ANSName | Only when `principalBinding.type` is `ENS_ENSIP26` |
